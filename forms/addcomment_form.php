...
class AddCommentForm extends moodleform {
    ...
    function definition() {
        ...
        $mform->addElement('hidden', 'jobid', $this->_customdata['jobid']); // issue id
        $mform->setType('jobid', PARAM_INT);

        $mform->addElement('editor', 'comment_editor', get_string('comment', 'jobtracker'), $this->editoroptions);
    ...
    function set_data($defaults) {
        ...
        $defaults = file_prepare_standard_editor($defaults, 'comment', $this->editoroptions, $this->context, 'mod_jobtracker', 'jobcomment', $defaults->id);
