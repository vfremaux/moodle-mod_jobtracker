...
if (!function_exists('compile_mail_template')){
    ...
    */
    function jobtracker_compile_mail_template($template, $infomap, $module, $lang = '') {
        global $USER;
        
        if (empty($lang)) $lang = $USER->lang; 
        $lang = substr($lang, 0, 2); // be sure we are in moodle 2
        
        $notification = implode('', jobtracker_get_mail_template($template, $module, $lang));
...
if (!function_exists('get_mail_template')){
    ...
    */
    function jobtracker_get_mail_template($virtual, $modulename, $lang = ''){
        global $CFG;
