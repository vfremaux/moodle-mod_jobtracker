...
elseif ($action == 'doaddelement'){
    ...
    if(!count($errors)){
        ...
        if (!$form->elementid = $DB->insert_record('jobtracker_element', $element)){
            print_error('errorcannotcreateelement', 'jobtracker');
        }

        $elementobj = jobtracker_getelement(null, $form->type);
...
elseif ($action == 'editelement'){
    ...
    if ($form->elementid != null){
        $element = jobtracker_getelement($form->elementid);
        ...
        include $CFG->dirroot.'/mod/classes/jobtrackercategorytype/editelement.html';
    } else {
...
if ($action == 'doupdateelement'){
    ...
    if (empty($form->elementid)){
        print_error('errorelementdoesnotexist', 'jobtracker');
    }
    ...
    if(!count($errors)){
        ...
        if (!$DB->update_record('jobtracker_element', $element)){
            print_error('errorcannotupdateelement', 'jobtracker');
        }
    } else {
        $form->action = 'doupdateelement';
        include $CFG->dirroot.'/mod/jobtracker/classes/jobtrackercategorytype/editelement.html';
...
if ($action == 'deleteelement'){
    ...
    if(!jobtracker_iselementused($jobtracker->id, $elementid)){ 
        if (!$DB->delete_records ('jobtracker_element', 'id', $elementid)){    
            print_error ('errorcannotdeletelement', 'jobtracker');
        }
...
if ($action == 'moveelementoptionup'){
    ...
    if ($sortorder > 1){
        ...
        if (!$DB->update_record('jobtracker_elementitem', addslashes_recursive($option))){
            print_error('errorupdateelement', 'jobtracker');
        }
        if (!$DB->update_record('jobtracker_elementitem', addslashes_recursive($previousoption))){
            print_error('errorupdateelement', 'jobtracker');
        }
    }    
    echo $OUTPUT->heading(get_string('editoptions', 'jobtracker'));
...
if ($action == 'addelement'){
    ...
    if(!jobtracker_iselementused($jobtracker->id, $elementid)){
        ...
        if (!insert_record ('jobtracker_elementused', $used)){    
            print_error('errorcannotaddelementtouse', 'jobtracker');
        }        
    } else {
        //Feedback message that element is already in use
        print_error('errorelementinuse', 'jobtracker', '', "view.php?id={$cm->id}&amp;what=manageelements");
    }        
}    
/****************************** remove an element from usable list **********************/
if ($action == 'removeelement'){
    $usedid = required_param('usedid', PARAM_INT);
    if (!$DB->delete_records ('jobtracker_elementused', 'elementid', $usedid, 'jobtrackerid', $jobtracker->id)){    
        print_error('errorcannotdeleteelementtouse', 'jobtracker');
    }
}    
?>